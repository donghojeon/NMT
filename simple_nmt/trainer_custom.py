...
class Trainer():
    ...
    def train(self, train, valid):
        ...
        print('train start - epoch : %d / %s' % (self.n_epochs, start_time))
        for idx in range(self.best['epoch'], self.n_epochs):  # Iterate from 1 to n_epochs
            if self.config.adam:
            ...
            # train_epoch
            avg_train_loss, avg_param_norm, avg_grad_norm = self.train_epoch( train, optimizer )
            # valid_epoch
            avg_valid_loss = self.validate(valid)
            # loss list save
            train_loss_list.append(avg_train_loss)
            valid_loss_list.append(avg_valid_loss)
            print('epoch: %d/%d\tloss_tran / valid:%.4e / %.4e\tcurrent_lr:%f' % (idx + 1,
                                                                                 self.n_epochs,
                                                                                 avg_train_loss,
                                                                                 avg_valid_loss,
                                                                                 current_lr
            ...
            if (self.lower_is_better and avg_valid_loss < best_loss) or (not self.lower_is_better and avg_valid_loss > best_loss):
                ...
                ## general training(n_epochs 보다 idx가 작을 경우)
                if self.config.n_epochs >= idx + 1:
                    model_fn = model_fn[:-1] + ['%02d_' % (idx + 1),
                                                'trainloss:%.2f_'  % (avg_train_loss),
                                                'validloss:%.2f'   % (avg_valid_loss)
                                                ] + [model_fn[-1]]
                ## reinforcement training
                else:
                    model_fn = model_fn[:-1] + ['%02d' % (idx + 1),
                                                'trainloss:%.2f'   % (avg_train_loss),
                                                'validloss:%.2f_RL'% (avg_valid_loss)
                                                ] + [model_fn[-1]]

                ## model save
                self.save_training('.'.join(model_fn))
            ## !! early stop 이 켜져있고, config.early_stop보다 오래 진전이 없으면 스탑
            else:
                lowest_after += 1
            ...
            # loss 향상이 없거나, 향상이 있어도 lr decay 시작 상수(10, config.lr_decay_start_at) 이상일 때 learning rate decay 적용(*0.5)
        ...
        # train, val losee return
        print(datetime.datetime.now() - start_time)  # 수행시간 측정
    ...
    def validate(self,
        ...
        with torch.no_grad():
            ...
            for idx, mini_batch in enumerate(valid):
                x, y = mini_batch.src, mini_batch.tgt[0][:, 1:]
                # |x| = (batch_size, length)
                # |y| = (batch_size, length)
                y_hat = self.model(x, mini_batch.tgt[0][:, :-1])
            ...
            # loss가 너무 큰경우, exp 에러남, exp(700) 까지만 계산 가능
            if avg_loss > 700:avg_loss = 700
#             print('valid_loss=%.4e PPL=%.2f' % (avg_loss, exp(avg_loss)))
